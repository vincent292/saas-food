...
function mapProduct(row: {
  ...
  return {
    ...
    whatsappImageUrl: row.whatsapp_image_url ?? undefined,
...
export const productService = {
  async listByRestaurant(restaurantId: string) {
    ...
    const { data, error } = await supabase
      .from("products")
      .select("id,restaurant_id,category_id,name,description,price,compare_at_price,prep_minutes,image_url,whatsapp_image_url,image_position_x,image_position_y,image_zoom,is_available,is_featured,product_kind,available_from,available_until,available_days,available_start_time,available_end_time,order_count,last_ordered_at,track_stock,sort_order")
