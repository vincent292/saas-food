...
declare
  ...
  v_requested_fulfillment_at timestamptz;
...
begin
  ...
  v_requested_fulfillment_at := nullif(p_order->>'requested_fulfillment_at', '')::timestamptz;
  v_order_type := (p_order->>'order_type')::order_type;
  ...
  if v_order_type = 'table' and v_requested_fulfillment_at is not null then
    raise exception 'scheduled-table-not-available' using errcode = '22023';
  end if;

  if (v_order_type = 'table' or v_requested_fulfillment_at is null)
    and not exists (
      select 1
