...
create table if not exists rider_renewal_requests (
  id uuid primary key default gen_random_uuid(),
  restaurant_rider_id uuid not null references restaurant_riders(id) on delete cascade,
  restaurant_id uuid not null references restaurants(id) on delete cascade,
  payment_amount numeric(10,2) not null default 30 check (payment_amount >= 0),
  payment_currency text not null default 'BOB',
  payment_qr_url text,
  payment_qr_note text,
  payment_proof_url text not null,
...
create index if not exists idx_rider_renewal_requests_restaurant_status
  on rider_renewal_requests(restaurant_id, status, created_at desc);

create index if not exists idx_rider_renewal_requests_rider
  on rider_renewal_requests(restaurant_rider_id, created_at desc);

create unique index if not exists idx_rider_renewal_requests_one_pending
  on rider_renewal_requests(restaurant_rider_id)
  where status = 'submitted';

create or replace function validate_rider_renewal_restaurant()
returns trigger
language plpgsql
as $$
begin
  if not exists (
    ...
    where rr.id = new.restaurant_rider_id
      and rr.restaurant_id = new.restaurant_id
  ) then
    raise exception 'restaurant_rider_mismatch';
  end if;
...
drop trigger if exists rider_renewal_requests_restaurant_guard on rider_renewal_requests;
create trigger rider_renewal_requests_restaurant_guard
  before insert or update on rider_renewal_requests
  for each row execute function validate_rider_renewal_restaurant();

drop trigger if exists rider_renewal_requests_updated_at on rider_renewal_requests;
create trigger rider_renewal_requests_updated_at
  before update on rider_renewal_requests
  for each row execute function set_updated_at();

alter table rider_renewal_requests enable row level security;

drop policy if exists "members read rider renewal requests" on rider_renewal_requests;
create policy "members read rider renewal requests" on rider_renewal_requests
  for select using (
    is_superadmin()
    or has_restaurant_role(restaurant_id, array['restaurant_admin']::app_role[])
  );

drop policy if exists "admins create rider renewal requests" on rider_renewal_requests;
create policy "admins create rider renewal requests" on rider_renewal_requests
...
drop policy if exists "superadmin manages rider renewal requests" on rider_renewal_requests;
create policy "superadmin manages rider renewal requests" on rider_renewal_requests
  for all using (is_superadmin()) with check (is_superadmin());

grant select, insert, update on rider_renewal_requests to authenticated;
grant select, insert, update on rider_renewal_requests to service_role;
